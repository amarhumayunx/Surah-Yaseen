...
/// Anchored adaptive banner ad widget for LanguageScreen
/// Follows Google Mobile Ads SDK best practices for anchored adaptive banners
class LanguageScreenBannerAdWidget extends StatefulWidget {
  const LanguageScreenBannerAdWidget({super.key});

  @override
  State<LanguageScreenBannerAdWidget> createState() =>
      _LanguageScreenBannerAdWidgetState();
}

class _LanguageScreenBannerAdWidgetState
    extends State<LanguageScreenBannerAdWidget> {
  BannerAd? _bannerAd;
  ...
  String get _adUnitId {
    ...
    // Production ad unit ID for LanguageScreen
    return AdUnitIds.languageScreenBanner;
  }
    ...
    _bannerAd = BannerAd(
      ...
      listener: BannerAdListener(
        onAdLoaded: (ad) {
          ...
          debugPrint('Language screen banner ad was loaded.');
          if (mounted) {
            setState(() {
        ...
        onAdFailedToLoad: (ad, err) {
          ...
          debugPrint('Language screen banner ad failed to load with error: $err');
          ...
          if (err.code == 3) {
            debugPrint(
                'No ad available (No fill). This is normal for new ad units.');
            debugPrint(
                'The ad will appear once ads become available in your region.');
          }
        },
        onAdOpened: (Ad ad) {
          // Called when an ad opens an overlay that covers the screen.
          debugPrint('Language screen banner ad was opened.');
        },
        onAdClosed: (Ad ad) {
          // Called when an ad removes an overlay that covers the screen.
          debugPrint('Language screen banner ad was closed.');
        },
        onAdImpression: (Ad ad) {
          // Called when an impression occurs on the ad.
          debugPrint('Language screen banner ad recorded an impression.');
        },
        onAdClicked: (Ad ad) {
          // Called when a click event occurs on the ad.
          debugPrint('Language screen banner ad was clicked.');
        },
        onAdWillDismissScreen: (Ad ad) {
          // iOS only. Called before dismissing a full screen view.
          debugPrint('Language screen banner ad will be dismissed.');
        },
      ),
    );
