...
class ListenAudioWithTranslationRukuSecondAudioPlayer extends StatefulWidget {
  final String? title;
  ...
  const ListenAudioWithTranslationRukuSecondAudioPlayer({
    Key? key,
    this.title,
  ...
  @override
  State<ListenAudioWithTranslationRukuSecondAudioPlayer> createState() =>
      _ListenAudioWithTranslationRukuSecondAudioPlayerState();
}

class _ListenAudioWithTranslationRukuSecondAudioPlayerState
    extends State<ListenAudioWithTranslationRukuSecondAudioPlayer>
  ...
  void _initVerses() {
    ...
    // Filter verses based on the actual verse numbers, not the indices
    if (widget.startVerse > 0 || widget.endVerse >= 0) {
      // Find the indices that match the desired verse numbers
      int startIndex = verseKeys.indexWhere((key) => key >= widget.startVerse);
      if (startIndex == -1) startIndex = 0;

      int endIndex = widget.endVerse < 0
          ? verseKeys.length - 1
          : verseKeys.lastIndexWhere((key) => key <= widget.endVerse);
      if (endIndex == -1) endIndex = verseKeys.length - 1;

      // Make sure endIndex is not less than startIndex
      endIndex = max(endIndex, startIndex);

      print("Filtering verses from number ${widget.startVerse} to ${widget.endVerse}");
      print("Corresponding to indices $startIndex to $endIndex");

      verseKeys = verseKeys.sublist(startIndex, endIndex + 1);
      arabicVerseTexts = arabicVerseTexts.sublist(startIndex, endIndex + 1);

      print("Filtered to ${verseKeys.length} verses from number ${verseKeys.first} to ${verseKeys.last}");
    }

    // Print the actual verses for debugging
    for (int i = 0; i < arabicVerseTexts.length; i++) {
      print(
        "Verse ${verseKeys[i]}: ${arabicVerseTexts[i].substring(0, min(30, arabicVerseTexts[i].length))}...",
      );
    }
  }
